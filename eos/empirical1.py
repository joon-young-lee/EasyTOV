...
e_pure_neutron = lambda n: n * (cgs.GeV_m_n + 2 ** (2/3) * cgs.E_0 * (n/cgs.n_0) **(2/3)
                                + cgs.A/2 * (n/cgs.n_0) + cgs.B/(cgs.sigma + 1) * (n/cgs.n_0) ** cgs.sigma
                                + n/cgs.n_0 * (cgs.S_0 - (2.0 ** (2/3) - 1) * cgs.E_0))
...
np.savetxt(f'./eos/empirical1.dat', data, fmt='%.6E', delimiter=' ', 
                header='', comments='')
